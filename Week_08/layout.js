function layout(element) {
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        if (itemStyle.flex) {
            flexLine.push(item);
        } else if (style.flexWrap === 'nowrap' && isAuthMainSize) {
    ...
    // 计算主轴
    if (style.flexWrap === 'nowrap' || isAuthMainSize) {
    ...
    flexLines.forEach(items => {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            if (align === "stretch") {
                ...
                itemStyle[crossSize] = crossSign * (itemStyle[crossSign] - itemStyle[crossStart])
            }
        }
